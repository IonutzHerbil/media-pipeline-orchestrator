...
public class SceneIndexer extends BaseStage {
  ...
  protected StageResult run(PipelineContext ctx) {
    ...
    if (!probe.ok()) {
      return StageResult.fail(name(), "ffprobe failed: " + probe.stderr(), elapsed(t));
    }
    ...
    for (int i = 0; i < ranges.size(); i++) {
      ...
      String level;
      int crf;
      if (avg < COMPLEXITY_LOW) {
        level = "low";
        crf = CRF_LOW;
      } else if (avg > COMPLEXITY_HIGH) {
        level = "high";
        crf = CRF_HIGH;
      } else {
        level = "medium";
    ...
    try {
      new ObjectMapper().writerWithDefaultPrettyPrinter().writeValue(outputPath.toFile(), result);
    } catch (IOException e) {
      return StageResult.fail(
          name(), "Failed to write scene_analysis.json: " + e.getMessage(), elapsed(t));
    }
  ...
  private int countPackets(List<double[]> packets, double start, double end) {
    return (int) packets.stream().filter(p -> p[0] >= start && p[0] < end).count();
  }
