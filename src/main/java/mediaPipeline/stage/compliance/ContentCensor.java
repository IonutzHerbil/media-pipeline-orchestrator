  ...
  protected StageResult run(PipelineContext ctx) {
    ...
    if (!reportPath.toFile().exists())
      return StageResult.fail(
          name(), "safety_report.json missing: SafetyScanner must run first", elapsed(t));
    ...
    try {
      ...
      return StageResult.fail(
          name(), "Could not read safety_report.json: " + e.getMessage(), elapsed(t));
    }
    ...
    for (JsonNode flag : flags) {
      ...
      if ("bleep_audio".equals(action))
        bleepFilters.add(
            String.format("volume=enable='between(t,%.3f,%.3f)':volume=0", start, end));
      else if ("EPILEPSY_RISK".equals(category))
        blurFilters.add(String.format("boxblur=10:enable='between(t,%.3f,%.3f)'", start, end));
    }
    ...
    if (!bleepFilters.isEmpty()) cmd.addAll(List.of("-af", String.join(",", bleepFilters)));

    if (!blurFilters.isEmpty())
      cmd.addAll(
          List.of(
              "-vf",
              String.join(",", blurFilters),
              "-c:v",
              "libx264",
              "-preset",
              "fast",
              "-crf",
              "23"));
    else cmd.addAll(List.of("-c:v", "copy"));

    cmd.addAll(List.of("-c:a", "aac", "-b:a", "128k", output.toAbsolutePath().toString()));
