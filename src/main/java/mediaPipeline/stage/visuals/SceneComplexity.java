  ...
  protected StageResult run(PipelineContext ctx) {
    ...
    if (analysisPath == null) {
      return StageResult.fail(
          name(), "scene_analysis_path missing: SceneIndexer must run first", elapsed(t));
    }
    JsonNode root;
    try {
      ...
      return StageResult.fail(
          name(), "Failed to read scene_analysis.json: " + e.getMessage(), elapsed(t));
    }

    JsonNode scenes = root.get("scenes");
    if (scenes == null || !scenes.isArray() || scenes.isEmpty()) {
      return StageResult.fail(name(), "scene_analysis.json has no scenes", elapsed(t));
    }
    ...
    double avgCrf = crfValues.stream().mapToInt(Integer::intValue).average().orElse(23);

    log.info(
        "CRF distribution → 18:{} 23:{} 26:{} | dominant:{} avg:{}",
        crf18,
        crf23,
        crf26,
        dominantCrf,
        String.format("%.1f", avgCrf));
    ctx.put("avg_crf", avgCrf);
    ctx.put("suggested_crf", dominantCrf);
    ctx.put("complexity_profile", crfValues);

    log.info("Encoding profile → CRF {} ({} scenes analyzed)", dominantCrf, scenes.size());
